...
def get_page_links(
    ...
    :param params: params to put in the resulting url
    ...
    if has_next_prev and page_current > 0:
        links.append(
            _get_page_link(
                u, params, offset - per_page, per_page, gettext("Previous"), CLASS_PREV
            )
        )
    else:
    ...
    if has_next_prev and page_current + 1 != page_total and page_total > 1:
        links.append(
            _get_page_link(
                u, params, offset + per_page, per_page, gettext("Next"), CLASS_NEXT
            )
        )
    else:
