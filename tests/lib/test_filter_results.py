...
from hubgrep.frontend_blueprint.routes.index import SearchForm, Checkbox, get_form_datetime_in_utc
...
class TestFilterResults:
    ...
    @staticmethod
    def _get_form_mock(search_phrase: str = "find_me", gh_checked: bool = False, gl_checked: bool = False):
        checkboxes = {"gh": Checkbox("gh", 1, "github", gh_checked),
                      "gl": Checkbox("gl", 1, "gitlab", gl_checked)}
        return SearchForm(search_phrase, checkboxes, False, False)
    ...
    def test_filter_created_before(self):
        results = self._get_results_mock()
        form = self._get_form_mock()
        date = get_form_datetime_in_utc(GL_CREATED, DATE_FORMAT) - timedelta(days=1)
    ...
    def test_filter_updated_after(self):
        results = self._get_results_mock()
        form = self._get_form_mock()
        date = get_form_datetime_in_utc(GL_UPDATED, DATE_FORMAT) - timedelta(days=1)
